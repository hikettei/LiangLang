  ...
  (cond
    ...
    ((equal command "run")
     (let ((vm (liang.lvm:load-lvm-structure (car args))))
       (liang-runvm vm)))
    ((equal command "iseq")
     (let ((vm (liang-makevm-byfile (car args))))
       (liang.lvm:epochvm vm)))
    ((equal command "eval")
    ...
    ((equal command "profile")
     (time (liang.lvm:load-lvm-structure (car args)))
     (sb-profile:profile "LIANG.LVM" "LIANG" "LIANG.COMPILER")
     (time (liang-runvm (liang.lvm:load-lvm-structure (car args))))
     (sb-profile:report))
    ((equal command "disasm")
     (let ((vm (liang.lvm:load-lvm-structure (car args))))
       (liang.lvm:epochvm vm))))
  (princ #\newline))
